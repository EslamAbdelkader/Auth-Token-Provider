...
class AuthTokenProviderTest {
    ...
    private val isLoggedInObservable = BehaviorRelay.createDefault<Boolean>(true)
    ...
    private val refreshAuthToken = defer {
    ...
    fun `No refresh for logged out users`() {
        ...
        val isLoggedInObservable = BehaviorRelay.createDefault<Boolean>(false)
    ...
    fun `First subscriber triggers a refresh`() {
        ...
        val refreshAuthToken =
            Single.just(AuthToken(token, ZonedDateTime.now().plusMinutes(Companion.TOKEN_VALID_MINUTES)))
